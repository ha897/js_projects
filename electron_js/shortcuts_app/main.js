...
ipcMain.on("reload-shortcuts", (event,renderer_id) => {
//   const senderId = event.sender.id;
  ...
  BrowserWindow.getAllWindows().forEach((win) => {
    ...
    //   console.log(`no send to ${win.webContents} with id ${win.webContents.id}`);
      win.webContents.send("reload-shortcuts", renderer_id);
    // }
  });
});
...
function addShortcutFile(event, shortcutFormula, filePath, saveInDB = true) {
  ...
  if (isAlreadyRegistered && saveInDB === true) {
    dialog
      .showMessageBox(shortcutFileWin || mainWin, {
        type: "question",
...
function addShortcutCommand(
  ...
  if (isAlreadyRegistered && saveInDB === true) {
    dialog
      .showMessageBox(shortcutCommandWin || mainWin, {
        type: "question",
        title: "Shortcut Exists",
        message: `Shortcut ${shortcutFormula} already exists.\nReplace it?`,
        buttons: ["Yes", "Cancel"],
...
function createShortcutCommandWin() {
  ...
  // shortcutCommandWin.webContents.openDevTools();
...
// devaper
ipcMain.on("print-message", (event, renderer, message) => {
  console.log(`Message from renderer ${renderer}, message is: ${message}`);
});
